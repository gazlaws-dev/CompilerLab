	...
	switch((t->nodetype)){
		case tFCALL:{
				...
				while(arg!=NULL){
					...
					p = codeGen(arg,fp);//here, you get a func call that needs to save IT's own registers, and that overwrites the tempCount
					fprintf(fp,"PUSH R%d\n",p);
					arg=arg->arglist;
					}
				...
				tempreg=tempCount;
				restoreReg(fp);
				//tempreg is one more than used, and now I've used tempreg itself
				tempreg=reg+1;
